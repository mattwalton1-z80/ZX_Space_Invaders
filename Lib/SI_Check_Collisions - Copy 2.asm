...
check_alien_row_ar1:
	...
	jr z,check_alien_row_ar2_1	; if row doesn't exist (Z flag is set when bit tested is zero), skip to next row
	
	; if it does, get alien position
	ld 	a,(ix+1)				; get high byte of alien position
	and $F8						; mask first 3 bits of high byte so x (row) position resets to top pixel line in the row
	ld 	d,a
	ld 	a,(ix+2)				; get low byte of alien position
	and $E0						; make first 5 bits of low byte so y (column) position resets to beginning of row
	ld 	e,a
;	call upde_row				; return the address in DE of the next screen row directly below - not sure if this is needed...
					
	; prep bullet position
	ld 	a,l						; make first 5 bits 0 so bullet x (row) position resets to beginning of row
	and $E0
	ld 	l,a
...
bullet_offset_0:
	...
	jr bullet_posn_calc
bullet_offset_1:
	ld a,2					; add 2 pixel (since column position is already shifted right by 1 char)
	jr bullet_posn_calc
bullet_offset_2:
	ld a,4					; add 4 pixels (since column position is already shifted right by 1 char)
	jr bullet_posn_calc
bullet_offset_3:
	ld a,6					; add 6 pixels (since column position is already shifted right by 1 char) 
